...
void AutoTrader::OrderBookMessageHandler(
    ...
    if (instrument == Instrument::FUTURE) {
        ...
        unsigned long newAskPrice = askPrices[0] + PROFIT;
        unsigned long newBidPrice = bidPrices[0] - PROFIT;
        ...
        if (allowSell && mAskId == 0 && newAskPrice != 0) {
            SendInsertOrder(++mNextMessageId, Side::SELL, newAskPrice, (POSITION_LIMIT + mPosition) / 2, Lifespan::GOOD_FOR_DAY);
            mAskPrice = newAskPrice;
            mAskId = mNextMessageId;
            mAsks.emplace(mAskId);
        }
        if (allowBuy && mBidId == 0) {
            SendInsertOrder(++mNextMessageId, Side::BUY, newBidPrice, (POSITION_LIMIT - mPosition) / 2, Lifespan::GOOD_FOR_DAY);
