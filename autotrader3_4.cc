...
constexpr double TRANSACT_FEE = 0.0001;
...
void AutoTrader::OrderBookMessageHandler(
    ...
    if (instrument == Instrument::FUTURE) {
        ...
        unsigned long newAskPrice = (askPrices[0] != 0) ? (askPrices[0] * (1 + TRANSACT_FEE)) : 0;
        unsigned long newBidPrice = (bidPrices[0] != 0) ? (bidPrices[0] * (1 - TRANSACT_FEE)) : 0;
        ...
        // to prevent overlimit mPosition: position in ETF by rules: you can only have -100 <= mPosition <= 100
        ...
        // create new limit order with the new setted price
